...
```bash
mise run run          # = go run ./cmd/rawlens
# 然后浏览器开 http://localhost:9090
# 测试：curl -X POST localhost:8080/hi -d 'hello'
```
...
交叉编译一个 Linux 单二进制（无运行时依赖，前端页面已 `embed` 进二进制）：

```bash
mise run build-linux        # 产物 bin/rawlens-linux-amd64
...
internal/
  ...
  store/store.go             内存环形存储 + CapturedRequest 类型
  capture/capture.go         裸 TCP 抓包 + 原始解析（保真的核心）
  capture/tls.go             TLS 配置 / 自签名证书生成
  dashboard/dashboard.go     面板 JSON API + 托管前端
web/                         前端（编进二进制，部署无需额外文件）
  embed.go                   go:embed 静态资源
  index.html / styles.css / app.js
```

后端依赖方向：`capture → store`、`dashboard → store, web`、`main → config + 三者`。
配置外置（`config.yaml`），前端内嵌（`go:embed`）——部署时带上二进制 + 一个 yaml 即可。
前端是零构建的原生 HTML/CSS/JS（磷光示波器主题，RAW / HEADERS / BODY / HEX 四视图），改完直接重新编译即可。
