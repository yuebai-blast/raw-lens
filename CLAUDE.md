...
**发布与镜像**：两条独立的 tag 触发流水线，均由 `v*.*.*` tag 触发、互不干扰。`release.yml`（goreleaser）出各平台二进制与 GitHub Release；`image.yml` 用 buildx 构建 `linux/amd64,arm64` 多架构镜像推到 `ghcr.io/yuebai-blast/raw-lens`（登录用内置 `GITHUB_TOKEN`，`latest` 仅 tag 触发且非预发布才打）。镜像由仓库根 `Dockerfile` 三阶段构建：`node:20.18.1` 跑 `pnpm build` 出前端 → `golang:1.26.4` 把产物 `COPY` 进 `web/dist` 后 `CGO_ENABLED=0` 编译（embed 机制同上）→ `distroless/static` 非 root（uid 65532）运行，`WORKDIR /data`、`rawlens.db` 落该目录。三处基础镜像版本必须与 `mise.toml` 逐字对齐。改 Dockerfile 后 `mise run image` 本地验证；PR 阶段 `ci.yml` 的 `image-build` job 会兜底。本项目**不做服务器自动部署**。
